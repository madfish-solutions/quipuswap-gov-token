 ...
 *)
function has_expired(const default_expiry: seconds; const user_expiry_opt: option(seconds); const permit_info: permit_info) : bool is
  block {
...
function sender_check (const expected_user : address; const store : quipu_storage; const full_param : quipu_action; const err_message : string) : quipu_storage is
...
function delete_expired_permits(const default_expiry: seconds; const user: address; const permits: permits) : permits is
  case Big_map.find_opt(user, permits) of
  | None -> permits
  | Some(user_permits) -> block {
      const updated_map: map (blake2b_hash, permit_info) =
        Map.fold(
          function(const acc: map (blake2b_hash, permit_info); const keyValue: (blake2b_hash * permit_info)): map (blake2b_hash, permit_info) is
      ...
      const updated_user_permits: user_permits = user_permits with record [permits = updated_map]
...
function check_duplicates(const default_expiry: seconds; const user_expiry_opt: option(seconds); const user_permits: user_permits; const permit: blake2b_hash) : unit is
...
function insert_permit(const default_expiry: seconds; const user: address; const permit: blake2b_hash; const permits: permits) : permits is 
  block {
    const user_permits: user_permits =
      case Big_map.find_opt(user, permits) of
        Some(user_permits) -> user_permits
...
function add_permit(const parameter: permit_param; const store: quipu_storage) : return is
  block {
    ...
    if (Crypto.check (key, signature, to_sign)) then
      store with record
        [
          ...
          permits = delete_expired_permits(store.default_expiry, issuer, insert_permit(store.default_expiry, issuer, permit, store.permits))
        ]
    else block {
 ...
 *)
function set_user_default_expiry(const user : address; const new_expiry : seconds; const permits : permits) : permits is
  block {
    const user_permits: user_permits =
      case Big_map.find_opt(user, permits) of
      | Some(user_permits) -> user_permits
 ...
 *)
function set_permit_expiry_with_check(const permit_info : permit_info; const new_expiry : seconds) : option(permit_info) is
  block {
    const permit_age: int = Tezos.now - permit_info.created_at;
  } with if permit_age >= int(new_expiry) then
            (None : option(permit_info))
          else
            Some(permit_info with record [expiry = Some(new_expiry)])
 ...
 *)
function set_permit_expiry(const user : address; const permit : blake2b_hash; const new_expiry : seconds; const permits : permits; const default_expiry : seconds) : permits is
 ...
 * nothing happens.
 *)
function set_expiry(const param: set_expiry_param; const store : quipu_storage; const full_param: quipu_action) : return is
  block {
    ...
    const updated_store : quipu_storage = sender_check(owner, store, full_param, "NOT_PERMIT_ISSUER");
  ...
  } with ((nil : list(operation)), updated_store with record [permits = updated_permits])
