...
function update_admin(var s : quipu_storage; const new_admin : address) : quipu_storage is
  block {
...
function update_minter(var s : quipu_storage; const param : update_minter_param) : quipu_storage is
  block {
    if Tezos.sender =/= s.admin then
      failwith("NOT_ADMIN")
    else
    ...
    if (param.percent < 1n) or (param.percent > 100n) then
      failwith("WRONG_PERCENT")
    else
      skip;
